    ...
    try:
        ...
        for m in messages:
            ...
            for rule in matched_rules:
                ...
                print(f"[DEBUG] Found action {action.id} for rule '{rule.rule_name}'. Fetching signature...", file=sys.stderr)
                ...
                # Try to fetch Gmail signature first
                gmail_signature = ''
                try:
                    print(f"[DEBUG] Attempting to fetch Gmail signature...", file=sys.stderr)
                    ...
                    if not gmail_signature:
                         for sendas in sendas_list.get('sendAs', []):
                            if sendas.get('sendAsEmail') == to_addr:
                                ...
                                break
                    if gmail_signature:
                        print(f"[DEBUG] Using Gmail signature for user {user.username}", file=sys.stderr)
                except Exception as e:
                    print(f"[DEBUG] Failed to fetch Gmail signature for user {user.username}: {e}", file=sys.stderr)

                signature_html = gmail_signature or ''
